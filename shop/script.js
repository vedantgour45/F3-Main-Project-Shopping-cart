...
function showItems(Arr) {
  ...
  <div class="info">
    <div style="margin-bottom: 10px; font-weight:500">${ele.title}</div>
    <div class="row">
      <div class="price" style="font-weight:bold">$${ele.price}</div>
    ...
    <div style='margin-top:10px;' class="row">Rating: ${generateStarRating(
      ele.rating.rate,
      ele.rating.count
    )}</div>
  </div>
...
function generateStarRating(ratingValue, ratingCount) {
  const maxRating = 5; // Maximum rating value
  let starsHTML = "";

  // Generate the full yellow stars
  for (let i = 0; i < Math.floor(ratingValue); i++) {
    starsHTML +=
      '<i class="fas fa-star" style="color: #F0C31F; font-size: 17px; "></i>';
  }

  // Generate the half star if necessary
  if (ratingValue % 1 !== 0) {
    starsHTML +=
      '<i class="fas fa-star-half" style="color: #F0C31F; font-size: 17px;"></i>';
  }

  // Generate the empty stars
  const remainingStars = maxRating - Math.ceil(ratingValue);
  for (let i = 0; i < remainingStars; i++) {
    starsHTML +=
      '<i class="far fa-star" style="color: grey; font-size: 17px;"></i>';
  }
